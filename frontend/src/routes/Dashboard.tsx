    ...
    return (
      <div className="roi-in mx-auto mt-10 max-w-lg rounded-lg border border-border2 bg-panel p-8 text-center">
        ...
        <p className="mb-5 text-sm text-gray-400">
          Crea uno en Configuración → pestaña "Lanzamientos" (nombre + fecha de inicio/fin) para ver el Panel ejecutivo.
        </p>
        ...
        <>
          <div className="flex flex-col gap-2.5">
            ...
            <div className="roi-stagger grid grid-cols-2 gap-2.5 md:grid-cols-3 xl:grid-cols-6">
              <KpiCard label="Compras aprobadas" value={formatNumber(summary.salesKpis.comprasAprobadas)} />
              <KpiCard label="Upgrades a VIP" value={formatNumber(summary.salesKpis.upgradesVip)} color="#c084fc" />
              <KpiCard label="Order bumps" value={formatNumber(summary.salesKpis.orderBumps)} color="#a855f7" />
              <KpiCard label="Leads gestionados" value={formatNumber(summary.salesKpis.leadsGestionados)} color="#818cf8" />
              <KpiCard label="Ticket promedio" value={formatUsd(summary.salesKpis.ticketPromedio)} color="#f59e0b" />
              <KpiCard label="Ingreso bruto" value={formatUsd(summary.salesKpis.ingresoBruto)} color="#34d399" />
              <KpiCard label="Neto del productor" value={formatUsd(summary.salesKpis.netoProductor)} color="#34d399" />
              <KpiCard label="Ingreso por upgrade" value={formatUsd(summary.salesKpis.ingresoPorUpgrade)} color="#c084fc" />
              <KpiCard label="Ingreso por bumps" value={formatUsd(summary.salesKpis.ingresoPorBumps)} color="#a855f7" />
              <KpiCard label="Pendiente por cobrar" value={formatUsd(summary.salesKpis.pendientePorCobrar)} color="#f59e0b" />
              <KpiCard label="Reembolsos y disputas" value={formatUsd(summary.salesKpis.reembolsosYDisputas)} color="#ef4444" />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2.5">
            ...
            <div className="roi-stagger grid grid-cols-3 gap-2.5">
              <KpiCard label="Cerrada" value={formatNumber(summary.embudoVentas.cerrada)} color="#34d399" />
              <KpiCard label="Ofertada" value={formatNumber(summary.embudoVentas.ofertada)} color="#38bdf8" />
              <KpiCard label="No ofertada" value={formatNumber(summary.embudoVentas.noOfertada)} color="#f59e0b" />
            </div>
          </div>

          <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">
            <ObjectionsWidget locationId={locationId} from={currentRange?.startDate ?? summary.launch.startDate} to={currentRange?.endDate ?? summary.launch.endDate} />
            <div className="rounded-[7px] border border-border bg-panel p-5">
              ...
              <div className="mt-4">
                <VolumeChart days={summary.salesVolume} />
              </div>
            </div>
          </div>
